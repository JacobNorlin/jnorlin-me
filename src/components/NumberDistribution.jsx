...
export default class NumberDistribution extends React.Component {
	...
	constructor(props){
		...
		this.initialState = [{label:"lol", values:[{x:"0", y:0}, {x:"1", y:0}, {x:"2", y:0}, {x:"3", y:0} , {x:"4", y:0}, {x:"5", y:0}, {x:"6", y:0}, {x:"7", y:0}, {x:"8", y:0}, {x:"9", y:0}]}],
		this.state =  {
	...
	startGen(event){
		...
		this.setState({stop:false, running: true});
	...
	stopGen(){
		this.setState({stop:true, running: false, data: this.initialState})
	}
	...
	render(){
		...
		/>
		<Button bsStyle="primary" disabled={this.state.running} onClick={this.startGen.bind(this)}>Start</Button>
		<Button bsStyle="primary" disabled={!this.state.running} onClick={this.stopGen.bind(this)}>Stop</Button>
		<BarChart
